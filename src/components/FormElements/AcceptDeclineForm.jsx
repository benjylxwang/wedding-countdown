            ...
            <Form.Group as={Row} controlId="formPartyLeader">
                ...
                <Col>
                    <ButtonGroup required>
                        <Button variant={(accepted[0] && accepted[0].accepted) ? "success" : "outline-success"} onClick={() => onAccept(0, input.leader)}>Accept</Button>
                        <Button variant={declined[0] ? "danger" : "outline-danger"} onClick={() => onDecline(0, input.leader)}>Decline</Button>
                    </ButtonGroup>
                </Col>
            </Form.Group>
            {
                input.guests.map((e, i) => 
                    <Form.Group as={Row} key={e} controlId="guest">
                        ...
                        <Col>
                            <ButtonGroup>
                                <Button variant={(accepted[i+1] && accepted[i+1].accepted) ? "success" : "outline-success"} onClick={() => onAccept(i+1, e)}>Accept</Button>
                                <Button variant={declined[i+1] ? "danger" : "outline-danger"} onClick={() => onDecline(i+1, e)}>Decline</Button>
                            </ButtonGroup>
                        </Col>
                    </Form.Group>
                )
